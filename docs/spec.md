...
- [x] **Windows-Installer** (`installer/traum-planer.iss`, Inno Setup) — kompiliert, installiert und gestartet getestet: App + gebündeltes Whisper-Modell (~500 MB) läuft ohne Adminrechte/UAC, mit Start-Menü- und Desktop-Verknüpfung. Optionale zweite Komponente für Ollama + `qwen2.5:3b` vollständig offline (kein "ollama pull" zur Laufzeit, Modell-Blobs werden mitinstalliert) ist fertig geskriptet, aber mangels eingecheckter Vendor-Dateien (mehrere GB, siehe `installer/vendor/ollama/README.md`) in dieser Runde nicht mit echten Ollama-Binärdaten kompiliert/getestet — die App-Komponente allein deckt bereits das komplette STT ab
- [ ] Linux-Vertriebsformat (AppImage, siehe offene Frage 3) — noch nicht gebaut, siehe Hinweis zu Linux-Verifikation oben
